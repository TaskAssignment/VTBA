...
//--------------------------------------------------------------------------------------------------------------------------------------------
	public static void xmlToTSV(String inputXMLPath, String inputXMLFileName, String outputTSVPath, String outputTSVFileName, 
			...
			while((s = br1.readLine()) != null) {
				if (s.startsWith("  <row ")){
					...
					if (i % showProgressInterval == 0)
						MyUtils.println(Constants.integerFormatter.format(i), indentationLevel);
				}//if (s.st...
			}//while((s = ....
			//		oldUsersRead = i;
			MyUtils.println("Writing TSV --------> Finished.", indentationLevel+1);
			MyUtils.println("Reading XML --------> Started ...", indentationLevel);
			MyUtils.println(i+" records processed.", indentationLevel);
			System.out.println();
			writer1.flush();
			writer1.close();
			br1.close();
